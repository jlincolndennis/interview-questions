...
> Persistent data stored on a client's machine. Local storage has no expiration (Session storage is cleared when a browser is closed), nor does it have a small size restriction, like cookies. No local storage information is automatically sent to the server.
...
> * Browser makes a DNS lookup to find the IP address
...
> * For every image, script tag etc., it checks the browser cache and makes new HTTP requests for anything missing
...
>
> **Integration tests** tend to test entire applications or large features, and ensure that the components built work well together. They usually require resources and are executed in realistic functioning environments. They also test that specific components behave correctly when connected to actual dependencies and resources.
